...
use super::types::{
    Context, DashboardGetNetworkResponse, DashboardNode, NodeConnection, NodeGetInfoResponse,
};

pub fn add_routing(router: &mut Router<Context>) {
    router.add_route("ping", Box::new(ping as fn(Context) -> RPCResponse<String>));
    router.add_route("node_getInfo", Box::new(node_get_info as fn(Context, (String,)) -> RPCResponse<NodeGetInfoResponse>));
    router.add_route(
